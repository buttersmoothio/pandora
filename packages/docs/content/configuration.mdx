...
Channels are how users interact with Pandora (Telegram, Web, Discord, etc.). A channel is enabled by being present in the config — remove it to disable.
...
| Field | Required | Description |
|-------|----------|-------------|
| `token` | Yes | Auth token that clients must provide |
