...
Endpoints for listing, reading, forking, and deleting conversation threads. See [API Conventions](/developer-guide/reference/conventions) for authentication and error format.
...
| `messages[].parts` | `Part[]` | Message content parts matching the [UIMessage](https://ai-sdk.dev/docs/reference/ai-sdk-core/ui-message) format. See [Message Part Types](/developer-guide/reference/message-parts). |
...
After forking, send a [`POST /api/chat`](/developer-guide/reference/chat#post-apichat) with the new thread's ID to continue the conversation from the fork point.
