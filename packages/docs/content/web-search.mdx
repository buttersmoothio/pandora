...
You can add your own search backend by creating a tool. See [Customization → Tools](/customization#tools) for the full guide.

Basic structure:

```typescript
// packages/pandora/src/tools/my-search.ts
import { defineTool, defineSearchTool } from "@pandora/core";
