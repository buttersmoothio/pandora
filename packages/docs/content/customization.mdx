...
Pandora uses a monorepo with two main packages:

```
packages/
├── core/                      # @pandora/core - Framework (don't modify)
│   └── src/
...
│
└── docs/                      # @pandora/docs - Documentation site
```

**Key insight:** All your customizations go in `packages/pandora/`. The `@pandora/core` package provides the framework - import from it but don't modify it.
