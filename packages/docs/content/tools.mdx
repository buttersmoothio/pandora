...
See the [Web Search Guide](/web-search) for detailed setup instructions.
...
See [Customization → Tools](/customization#tools) for a complete guide.

**Basic structure:**

```typescript
// packages/pandora/src/tools/weather.ts
import { tool } from "ai";
import { z } from "zod";
