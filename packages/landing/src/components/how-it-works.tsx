          ...
          {steps.map((step, i) => (
            <FadeIn key={step.number} delay={i * 0.1}>
              <div className="h-full rounded-2xl border border-border bg-surface p-8 transition-colors hover:border-accent/20">
                <span className="display-heading-medium font-display text-sm text-accent">
                  Step {step.number}
                </span>
                <h3 className="display-heading-medium font-display text-2xl text-foreground mt-2">
                  {step.title}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-muted">{step.description}</p>
                {step.code ? (
                  <pre className="mt-4 overflow-x-auto rounded-lg bg-background p-4 text-xs text-accent">
                    <code>{step.code}</code>
                  </pre>
                ) : null}
              </div>
