...
const ALLOWED_ATTRIBUTES: Record<string, Set<string>> = {
  a: new Set(["href"]),
  code: new Set(["className"]), // language-* for fenced code blocks
};
...
function transformNodes(
  ...
  for (const node of nodes) {
    ...
    if (/^h[1-6]$/.test(tag)) {
      result.push(
        element("b", transformNodes(node.children)),
        text("\n\n")
      );
      continue;
    }
    ...
    if (tag === "ol") {
      ...
      result.push(
        ...transformNodes(node.children, { listType: "ol", counter: start })
      );
      continue;
    }
    ...
    if (TELEGRAM_TAGS.has(tag)) {
      result.push(
        element(
          ...
          filterProperties(tag, node.properties)
        )
      );
      continue;
    }
